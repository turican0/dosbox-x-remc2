 ...
 *
 *  This program is distributed in the hope that it will be useful,disasm
...
static void SaveMemoryF(Bit16u seg, Bit32u ofs1, Bit32u num);
static void SaveMemoryF16(Bit16u seg, Bit32u ofs1, Bit32u num);
static void SaveMemoryF32(Bit16u seg, Bit32u ofs1, Bit32u num);
static void SaveMemoryBin(Bit16u seg, Bit32u ofs1, Bit32u num);
static void SaveMemoryBinx(Bit32u ofs1, Bit32u num);
static void DisAsm(Bit16u seg, Bit32u ofs1, Bit32u num);
...
bool ParseCommand(char* str) {
    ...
    if (command == "MEMDUMPF") { // Dump memory to file
        Bit16u seg = (Bit16u)GetHexValue(found, found); found++;
        Bit32u ofs = GetHexValue(found, found); found++;
        Bit32u num = GetHexValue(found, found); found++;
        SaveMemoryF(seg, ofs, num);
        return true;
    };

    if (command == "MEMDUMPF16") { // Dump memory to file
        Bit16u seg = (Bit16u)GetHexValue(found, found); found++;
        Bit32u ofs = GetHexValue(found, found); found++;
        Bit32u num = GetHexValue(found, found); found++;
        SaveMemoryF16(seg, ofs, num);
        return true;
    };

    if (command == "MEMDUMPF32") { // Dump memory to file
        Bit16u seg = (Bit16u)GetHexValue(found, found); found++;
        Bit32u ofs = GetHexValue(found, found); found++;
        Bit32u num = GetHexValue(found, found); found++;
        SaveMemoryF32(seg, ofs, num);
        return true;
    };
    ...
    if (command == "MEMDUMPBINX") { // Dump memory to file bineary
        Bit32u ofs = GetHexValue(found, found); found++;
        Bit32u num = GetHexValue(found, found); found++;
        SaveMemoryBinx(ofs, num);
        return true;
    };
    ...
    if (command == "SETSTATE") { // SetState
        int seg = GetHexValue(found, found); found++;
        /*Bit32u ofs = GetHexValue(found, found); found++;
        Bit32u num = GetHexValue(found, found); found++;*/
        SetState(seg);
        return true;
...
static void SaveMemoryF(Bit16u seg, Bit32u ofs1, Bit32u num) {
    ...
    while (num > 16) {
        sprintf(buffer, "", seg, ofs1);
        for (Bit16u x = 0; x < 16; x++) {
            Bit8u value;
            if (mem_readb_checked((PhysPt)GetAddress(seg, ofs1 + x), &value)) sprintf(temp, "%s", "?? ");
            else sprintf(temp, "0x%02X,", value);
            strcat(buffer, temp);
        }
        ofs1 += 16;
        num -= 16;
...
static void SaveMemoryF16(Bit16u seg, Bit32u ofs1, Bit32u num) {
    FILE* f = fopen("MEMDUMPF16.TXT", "wt");
    if (!f) {
        DEBUG_ShowMsg("DEBUG: Memory dump failed.\n");
        return;
    }
    ...
    while (num > 16) {
        sprintf(buffer, "", seg, ofs1);
        for (Bit16u x = 0; x < 16; x+=2) {
            Bit16u value;
            if (mem_readw_checked((PhysPt)GetAddress(seg, ofs1 + x), &value)) sprintf(temp, "%s", "?? ");
            else sprintf(temp, "0x%04X,", value);
            strcat(buffer, temp);
        }
        ofs1 += 16;
        num -= 16;
...
static void SaveMemoryF32(Bit16u seg, Bit32u ofs1, Bit32u num) {
    FILE* f = fopen("MEMDUMPF32.TXT", "wt");
    if (!f) {
        DEBUG_ShowMsg("DEBUG: Memory dump failed.\n");
        return;
    }
    ...
    while (num > 16) {
        sprintf(buffer, "", seg, ofs1);
        for (Bit16u x = 0; x < 16; x += 4) {
            Bit32u value;
            if (mem_readd_checked((PhysPt)GetAddress(seg, ofs1 + x), &value)) sprintf(temp, "%s", "?? ");
            else sprintf(temp, "0x%08X,", value);
            strcat(buffer, temp);
        }
        ofs1 += 16;
        num -= 16;
...
static void SaveMemoryBinx(Bit32u ofs1, Bit32u num) {
    ...
    for (Bitu x = 0; x < num;x++) {
        Bit8u val;
        if (mem_readb_checked(ofs1 + x, &val)) val = 0;
        fwrite(&val, 1, 1, f);
    }
...
static void DisAsm(Bit16u seg, Bit32u ofs1, Bit32u num) {
    ...
    sprintf_s(filename,maxnamesize,"DISASM-%04X-%08X.txt",seg,ofs1);
    FILE* f = fopen(filename, "wt");
    if (!f) {
        DEBUG_ShowMsg("DEBUG: disasm failed.\n");
        return;
    }
    ...
    while (num>0)
    {
        ...
        PhysPt start = GetAddress(seg, ofs1);
        char dline[200];
        //Bitu size = DasmI386(dline, start, reg_eip, cpu.code.big);
        Bitu size = DasmI386(dline, start, start, cpu.code.big);
        char* res = empty;
        if (showExtend) {
            res = AnalyzeInstruction(dline, false);
            if (!res || !(*res)) res = empty;
        ...
        */
        sprintf(temp, "%s ", dline);
        strcat(buffer, temp);
        Bit8u value;
        for (int j = 0;j < size;j++) {
            if (mem_readb_checked(GetAddress(seg, ofs1 + j), &value)) sprintf(temp, "%s", "?? ");
            else sprintf(temp, "%02X", value);
            strcat(buffer, temp);
        }
        sprintf(temp, "\n");
    ...
    fclose(f);
    DEBUG_ShowMsg("DEBUG: disasm success.\n");
}
